...
int main()
{
	...
	FILE_PT = fopen("C:\\Users\\YoungBeom Kim\\source\\repos\\AES_SW_trace\\message.txt", "r");
	...
	for (cnt_i = 0; cnt_i < TRACE_NUM; cnt_i++)
	{
		sprintf(str, "C:\\Users\\YoungBeom Kim\\source\\repos\\AES_SW_trace\\Trace%0.6d.txt", cnt_i);
		...
		for (cnt_j = 0; cnt_j < TRACE_LENGTH; cnt_j++)
		{
			...
			fread(&TraceTemp[cnt_i][cnt_j], sizeof(double), 1, FILE_Trace);
	...
	// 			{
	// 				byte temp = plaintext[cnt_k][cnt_s];
	...
	// 		}
	// 		sprintf(str, "/Users/yeongbeom/vscode/AESSW_CPA/CPA_Peak/CPA_%d_peak_%x.txt", cnt_i, guess_key[cnt_i]);
